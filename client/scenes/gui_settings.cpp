...
bool settings_tracking(const settings_context & ctx)
{
	...
	// Lying down mode: adjust view for playing while lying down
	list.push_back({
	        .id = "##lying_down",
	        .label = _C("setting name", "Lying Down Mode"),
	        .description = _("Adjusts the view for playing while lying down. Press Recenter after enabling."),
	        .ui = ui_kind::toggle,
	        .get_bool = [&config] { return config.lying_down_mode; },
	        .set_bool = [&config](bool v) { config.lying_down_mode = v; config.save(); },
	        .default_bool = default_config.lying_down_mode,
	});

	list.push_back({
	        .id = "##lying_height",
	        .label = _C("setting name", "Height"),
	        .description = _("Standing height used when lying down."),
	        .ui = ui_kind::slider,
	        .get_int = [&config] { return int(std::lround(config.lying_down_height * 100)); },
	        .set_int = [&config](int v) { config.lying_down_height = v / 100.f; config.save(); },
	        .v_min = 0,
	        .v_max = 250,
	        .fmt = "%d cm",
	        .default_int = int(std::lround(default_config.lying_down_height * 100)),
	        .enabled = [&config] { return config.lying_down_mode; },
	});
